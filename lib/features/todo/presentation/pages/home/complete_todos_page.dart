...
class CompleteTodosPage extends StatefulWidget {
  final TodoBloc todoBloc;

  const CompleteTodosPage({
    Key? key,
    required this.todoBloc,
  }) : super(key: key);

  @override
  State<CompleteTodosPage> createState() => _CompleteTodosPageState();
}

class _CompleteTodosPageState extends State<CompleteTodosPage> {

  @override
  void initState() {
    super.initState();
    widget.todoBloc.notifyGetCompleteTodos();
  }
  ...
  Widget _buildContent() {
    return StreamBuilder<List<ToDo>>(
        stream: widget.todoBloc.completeTodosStream,
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.active) {
            final todoList = snapshot.data;
            if (todoList == null || todoList.isEmpty) {
              return const Center(
                child: Text("Complete todos will be shown here"),
              );
            }
