    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `todo_table` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `text` TEXT NOT NULL, `isCompleted` INTEGER NOT NULL)');
...
class _$ToDoDao extends ToDoDao {
  ...
  Future<List<ToDo>> getConditionedTodos(bool isCompleted) async {
    return _queryAdapter.queryList(
        'SELECT * FROM todo_table WHERE isCompleted = ?1',
        mapper: (Map<String, Object?> row) => ToDo(
            id: row['id'] as int?,
            text: row['text'] as String,
            isCompleted: (row['isCompleted'] as int) != 0),
